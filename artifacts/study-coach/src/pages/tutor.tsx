...
export default function TutorPage() {
  const conversationQuery = useGetTutorConversation({ query: { queryKey: getGetTutorConversationQueryKey() } });
  ...
  const contextOptions = useMemo(
    () =>
      courses.flatMap((course) =>
        (course.topics.length ? course.topics.map((t) => t.name) : [""]).map(
          (topic) => `${course.name}${topic ? ` · ${topic}` : ""}`,
        ),
      ),
    [courses],
  );
  ...
  useEffect(() => {
    if (conversationQuery.data) {
      ...
      setConversationId(conversationQuery.data.conversationId);
    }
  }, [conversationQuery.data]);

  useEffect(() => {
    if (!context && contextOptions.length) setContext(contextOptions[0]);
  ...
  const submit = (value = draft) => {
    ...
    sendMessage.mutate(
      { data: { message, context: context || null, conversationId } },
      {
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_300px]">
        <section className="flex min-h-[600px] flex-col rounded-[24px] border border-border bg-card">
          ...
          <div className="flex-1 space-y-5 overflow-auto p-5 sm:p-7">
            ...
            ) : messages.map((message, index) => <div key={`${message.role}-${index}`} className={`flex gap-3 ${message.role === "student" ? "justify-end" : "justify-start"}`} data-testid={`message-${message.role}-${index}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-[13px] leading-relaxed sm:max-w-[72%] ${message.role === "student" ? "rounded-br-md bg-primary text-primary-foreground" : "rounded-bl-md bg-secondary text-primary"}`}>{message.role !== "student" && <div className="mb-2 flex items-center gap-1.5 font-mono-ui text-[9px] uppercase tracking-[0.14em] text-primary/50"><Sparkles className="h-3 w-3" /> Coach</div>}{message.message}</div></div>)}
          </div>
        ...
        <aside className="space-y-4"><div className="rounded-[22px] border border-border bg-card p-5"><div className="mb-4 flex items-center gap-2"><MessageCircle className="h-4 w-4 text-primary" /><p className="font-mono-ui text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Conversation context</p></div><label className="text-[11px] font-semibold text-primary" htmlFor="tutor-context">Studying</label>{contextOptions.length ? <select id="tutor-context" value={context} onChange={(event) => setContext(event.target.value)} data-testid="select-tutor-context" className="mt-2 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-[12px] text-primary outline-none focus:border-primary">{contextOptions.map((option) => <option key={option}>{option}</option>)}</select> : <p className="mt-2 text-[11px] text-muted-foreground">Add a course to give your tutor context.</p>}</div>
          <div className="rounded-[22px] border border-accent/30 bg-accent/15 p-5"><div className="mb-3 flex items-center gap-2 text-primary"><Lightbulb className="h-4 w-4" /><p className="font-mono-ui text-[10px] uppercase tracking-[0.15em]">Try a prompt</p></div><p className="text-[12px] leading-relaxed text-muted-foreground">Start with what you know. The tutor will meet you there.</p><div className="mt-4 space-y-2"><PromptButton text="Can you explain this a different way?" onClick={submit} /><PromptButton text="Give me a hint" onClick={submit} /><PromptButton text="Quiz me on this" onClick={submit} /></div></div>
